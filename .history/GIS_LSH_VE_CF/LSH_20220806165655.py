...
if __name__ == "__main__":
    ...
    nbits_lis,num_lis,d = [20],1,[1,2,3,4,5]
    lsh_cal = LSH(user_mx)
    start = 0
    for i in data_item:
        train_data = original_data.iloc[start:i]
        for nbits in num_lis:
            data['predict_latitude'],data['predict_longitude'],data['la_MAE'],data['long_MAE'] = 0,0,0,0
    ...
    plt.subplot(121)
    plt.plot(nbits_lis,mae,marker=style[0],markersize=14)
    plt.xlabel("nbits")
    plt.ylabel("MAE")
    plt.title("LSH")

    plt.subplot(122)
    plt.plot(nbits_lis,times,marker=style[1],markersize=14)
    plt.xlabel("nbits")
    plt.ylabel("时间")
    plt.title("LSH")
    fig=plt.gcf()
    fig.savefig('./GIS_LSH_VE_CF/picture/LSH_nbits.jpg',dpi=500)
    plt.show()
