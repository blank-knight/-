...
if __name__ == "__main__":
    ...
    nbits,num = 6,2
    lsh_cal = LSH(user_mx)
    hash_func = lsh_cal.hash_function(6,2)
    start = 0
    for i in data_item:
        train_data = original_data.iloc[start:i]
        lshTable = lsh_cal.lsh_table(trainData,hash_func,nbits,num) # 构建hash表
        te_lshTable = lsh_cal.lsh_table(testData,hash_func,nbits,num)
