...
if __name__ == "__main__":
    ...
    plt.subplot(121)
    plt.plot(nbits,mae,color = 'k',marker=style[0],markersize=9)
    plt.xlabel("buckets")
    plt.ylabel("MAE")
    plt.title("LSH误差随buckets的变化")
    plt.subplot(122)
    plt.plot(nbits,times,color = 'k',marker=style[1],markersize=9)
    plt.xlabel("buckets")
    plt.ylabel("时间")
    plt.title("LSH时间随buckets的变化")
    fig=plt.gcf()
    fig.savefig('../GIS_LSH_VE_CF/bw_picture/LSH_nbits_time_MAE.jpg',dpi=500)
    plt.show()
