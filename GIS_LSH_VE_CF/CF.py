...
class sim_cal():
    ...
    def fillSims(self,row):
        '''
            以行为单位进行导入处理
        '''
        userIds = pd.Series(row.index)
        print(userIds)
        row[:] = userIds.apply(self.calSim,args=(row.name,))
        return row

    def fill(self,short_mx,long_length):
        '''
            对短的稀疏矩阵进行填充扩容
        '''
        length = long_length-short_mx.shape[0]
        if len(short_mx.shape) == 1: # 如果是一维的，要单独处理
            short_mx = np.tile(short_mx,(long_length,1)) # 直接复制扩容
            return short_mx
        else:
            col_mean = np.mean(short_mx, axis=0) # 计算每一列平均值
            fill_vector = np.array(col_mean).reshape(1,2) # 构造填充向量
            fill_mx = np.tile(fill_vector,(length,1)) # 构造填充矩阵
            short_mx = np.r_[short_mx,fill_mx] # 将填充矩阵加在短的矩阵下面
            return short_mx
